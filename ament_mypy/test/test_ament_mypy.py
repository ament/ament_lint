...
def use_dir(tmpdir_factory):
    """Creates a sample data directory for testing.
    ...
    for tmp_file in files: tmp_file.write('', ensure=True)
    return use_me
...
def sample_errors(use_dir):
    filename = lambda x: str(use_dir.join(x))
...
def test__generate_mypy_report(mock_mypy_succ):
    ...
    assert ament_mypy.main._generate_mypy_report(file_names) == mock_mypy_succ.return_value
    ...
    args, _ = mock_mypy_succ.call_args
    assert len(args[0]) > 1
    assert any(args[0][i] == '--config-file' and args[0][i + 1] == 'a.ini' for i in range(len(args[0]) - 1))

    # Test if setting null cache dir prevents caching
    assert ament_mypy.main._generate_mypy_report(file_names, cache_dir = os.devnull) == mock_mypy_succ.return_value
    args, _ = mock_mypy_succ.call_args
    assert len(args[0]) > 1
    assert any(args[0][i] == '--cache-dir' and args[0][i + 1] == os.devnull for i in range(len(args[0]) - 1))
    assert '--no-incremental' in args[0]

    # Test if non-null cache dir uses caching
    assert ament_mypy.main._generate_mypy_report(file_names, cache_dir = '/tmp') == mock_mypy_succ.return_value
    args, _ = mock_mypy_succ.call_args
    assert len(args[0]) > 1
...
def test_main_success(mock_generate_report, use_dir):
    ...
    # Test that main returns failure when no files to pass to generate_report
    assert ament_mypy.main.main([]) == 0


def test_main_exclude(mock_generate_report, use_dir):
    mock_generate_report.return_value = ('', '', 0)
    # Test that excluding a file that was passed as an arg works
    assert ament_mypy.main.main([str(use_dir.join("01.py")), str(use_dir.join("02.py")), '--exclude', "02.py"]) == 0
    args, _ = mock_generate_report.call_args
    assert str(use_dir.join("01.py")) in args[0]
    assert str(use_dir.join("02.py")) not in args[0]
...
def test_main_xunit(mock_generate_report, mock_mypy_generate_fail, mocker, use_dir):
    mock_generate_report.return_value = ('', '', 0)
    ...
    assert ament_mypy.main.main([str(use_dir), '--xunit-file', str(use_dir.join('testgen.xunit.xml'))])
    assert mock_xunit.call_args[0][1].endswith('testgen')
    assert Path(use_dir.join('testgen.xunit.xml')).is_file()

    # Test that intermediate directories are handled
    assert ament_mypy.main.main([str(use_dir), '--xunit-file', str(use_dir.join('testdir', 'testgen'))])
    assert mock_xunit.call_args[0][1].endswith('testgen')
...
def test__get_xunit_content(mocker, sample_errors):
    ...
    xml = ament_mypy.main._get_xunit_content(errors, 'tst', [err.group('filename') for err in errors], .01)
    ...
    xml = ament_mypy.main._get_xunit_content([], 'tst', [err.group('filename') for err in errors], .01)
    root = ET.fromstring(xml)
